    ...
    for file_name in os.listdir(input_folder):
        if file_name.endswith(".svg"):
            ...
            output_svg_path = os.path.join(
                output_folder, f"sequence-{base_file_name}.svg"
            )
            print(len(df))
...
def process_single_file(svg_file_path, output_folder):
    ...
    base_file_name = os.path.splitext(os.path.basename(svg_file_path))[0]
    output_svg_path = os.path.join(output_folder, f"sequence-{base_file_name}.svg")

    dist = int(len(df) / 100)

    if len(df) > 2:
        df_combined = hierarchical_clustering(df, base_file_name, max_distance=dist)
        dataframe_to_svg(df_combined, output_svg_path)
    else:
        dataframe_to_svg(df, output_svg_path)
